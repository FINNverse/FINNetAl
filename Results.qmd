...
dcast(all_cors2[(is.na(k)|k == 3) & species2 != "all" & test_train == "test" & cv == "S0T0", rmse ,by = .(variable2, type2)], variable2~type2, value.var = "rmse", fun.aggregate = sum)
...
rmse_dt_all2 <- rbind(data.table(results_naive, type = "naive"), rmse_dt_all, use.names = T, fill = T)

rmse_dt_all3 <- rmse_dt_all2[,.(
  rmse = round(mean(sqrt((value.sim-value.obs)^2), na.rm=T),3)
), by = .(species2, variable2, type)]

dcast(rmse_dt_all3, variable2~type, value.var = "rmse", fun.aggregate = sum)
