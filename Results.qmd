...
for(i in c("pred_hybrid","pred_process")){
  ...
  # ggplot(p_dat[year > 500], aes(x = "sim", y = value, color = species2)) +
  #   geom_boxplot()+
  #   geom_jitter(data = obs_dt_year2, aes(x = "obs", y = value, color = species2))+
  #   coord_cartesian(ylim = c(0, NA)) +
  #   facet_wrap(species2~variable2, scales = "free_y", ncol = 6, strip.position = "top") +  # Remove label_parsed
  #   geom_blank(data=p_dat_dummy) + 
  #   theme_classic() +
  #   # ylab("Basal Area [m²/ha]") +
  #   scale_color_manual(
...
leg_p <- get_legend(p4+theme(legend.title = element_blank()))
grid.arrange(grobs = list(
  p1+ggtitle("a")+theme(legend.position = "none"), 
  p2+ggtitle("b")+theme(legend.position = "none"),
  # p3+ggtitle("c")+theme(legend.position = "none"),
  leg_p
  ),
  layout_matrix = rbind(
    c(1),
    c(2),
    c(3)
  ), widths = c(1), heights = c(0.5,.5,0.1))
