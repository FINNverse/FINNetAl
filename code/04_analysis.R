...
for(dataset in unique(names(pred_list))){
  ...
  for(simreal in names(pred_dataset)){
    ...
    for(i in 1:length(pred_dataset_simreal)){
      ...
      cat("\nstart", name, " ", i, "of", length(pred_dataset_simreal))
      dt <-
        rbindlist(
          list(
            pred_train = data.table(pred_l$pred$train$long$site, pred_obs = "pred", test_train = "train", simreal = simreal, dataset = dataset),
            pred_test = data.table(pred_l$pred$test$long$site, pred_obs = "pred", test_train = "test", simreal = simreal, dataset = dataset),
            obs_dt_train =
              data.table(
