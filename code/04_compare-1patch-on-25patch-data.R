...
result_folders = c("02_realdata", "02_simulated", "02_realdata_hybrid", "02_realdata_hybridTF1fixed")

all_models <- c(
  list.files("results/02_realdata/", full.names = T, recursive = T, pattern = "period7-1patch")
)
...
# noCV_models = all_models[grepl("pft-period35-1patch_S0_T2.pt", all_models)]
i_train = "test"
cairo_pdf(paste0("figures/02-results_noCV",i_train,"1patchOn25patch.pdf"), width = 14, height = 9)
all_cors_dt <- data.table()
all_cors_dt_plot <- data.table()
  i = noCV_models[1]
for(i in noCV_models){
  for(i_train in c("1patch-data","25patch-data")){
    cat("\nstarting", i,"\n", which(i == noCV_models), "of", length(noCV_models))
    if(i_train == "25patch-data"){
      i_data_dir = "data/BCI/CVsplits-realdata/genus-period7-25patches"
    }else{
      i_data_dir = NULL
    }
    name = paste0(names(pred_dt)[1],i_train)
    pred_dt = build_model_dt(pt_file = i, data_dir = i_data_dir)
    all_dt = pred_dt[[1]]
    ...
    p_dat <- all_dt2[test_train == i_train & species %in% top10_ba_species,.(value = mean(value,na.rm = T)), by = .(year,species,test_train, pred_obs, variable)]
    p_dat2 <- p_dat[test_train == i_train & !is.na(value)]
    p1 = ggplot(p_dat, aes(x = year, y = value))+
      rasterise(geom_point(aes(shape = pred_obs, color = factor(species), size = pred_obs), alpha = 0.5),dpi = 72)+
      rasterise(geom_line(data = p_dat2,  aes(linetype = pred_obs, color = factor(species))),dpi = 72)+
      rasterise(geom_line(aes(linetype = pred_obs, color = factor(species))),dpi = 72)+
      facet_wrap(~variable,scales = "free_y")+
      ggtitle(name)+
      scale_size_manual(values = c(3,1))
    print(p1)

    cor_agg <- all_dt2[test_train == i_train]
    if(period_length > 1){
    ...
    cor_dt <-
      cor_dt_in[!is.na(obs) & !is.na(pred) & test_train == i_train,.(
    ...
    cor_dt_species <- cor_dt_in[!is.na(obs) & !is.na(pred) & test_train == i_train,.(
      rmse = sqrt(mean((pred - obs)^2, na.rm = T)),
      spearmans_r = cor(pred, obs, method = "spearman"),
      obs_center = sum(range(obs, na.rm = T))/2,
      pred_center = sum(range(pred, na.rm = T)/2),
    ...
    cor_dt_plot <-
      cor_dt_in_plot[!is.na(obs) & !is.na(pred) & test_train == i_train,.(
        rmse = sqrt(mean((pred - obs)^2, na.rm = T)),
        spearmans_r = cor(pred, obs, method = "spearman"),
        obs_center = sum(range(obs, na.rm = T))/2,
        pred_center = sum(range(pred, na.rm = T)/2),
    ...
    p2a = ggplot(cor_dt_in[species %in% top10_ba_species], aes(x = pred, y = obs, color = factor(species)))+
      rasterise(geom_point(alpha = 0.1),dpi = 72)+
      rasterise(geom_smooth(method = "lm"),dpi = 72)+
      facet_wrap(~variable, scales = "free", ncol = 2)+
      ggtitle(paste0("per site:\n",name))+
    ...
    p2b = ggplot(cor_dt_species[species %in% c(top10_ba_species,"overall")], aes(y = species, x = variable))+
      ...
      theme_classic()+
      ggtitle(paste0("per site and species:\n",name))
    # print(p2b)
    # arrange p2a and p2b
    ...
    p3a = ggplot(cor_dt_in_plot[species %in% top10_ba_species], aes(x = pred, y = obs, color = factor(species)))+
      rasterise(geom_point(alpha = 0.8),dpi = 72)+
      rasterise(geom_smooth(method = "lm"),dpi = 72)+
      facet_wrap(~variable, scales = "free",ncol = 2)+
      ggtitle(paste0("whole plot:\n",name))+
      geom_abline(intercept = 0, slope = 1, color = "black", linetype = "dashed")+
      theme_minimal()+
      theme(legend.position = "bottom")
    # print(p3)
    ...
    p3b = ggplot(cor_dt_plot_species[species %in% c(top10_ba_species,"overall")], aes(y = species, x = variable))+
      geom_tile(aes(fill = spearmans_r), color = "white", linewidth = 3)+
      scale_fill_gradient(low = "white", high = "red", limits = c(0,1))+
      geom_text(aes(label = round(spearmans_r,2)), size = 3, color = "black")+
      facet_wrap(~test_train)+
      theme_classic()+
      ggtitle(paste0("whole plot and species:\n",name))
    # print(p3b)
    # arrange p3a and p3b
  ...
  p4 = ggplot(all_cors_dt, aes(y = paste(data,name), x = variable))+
    geom_tile(aes(fill = spearmans_r), color = "white", linewidth = 0)+
    scale_fill_gradient(low = "white", high = "red", limits = c(0,1))+
    geom_text(aes(label = round(spearmans_r,2)), size = 3, color = "black")+
    facet_wrap(~test_train)+
    theme_classic()+
