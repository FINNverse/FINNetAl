...
lossvars_stand = c("ba", "trees", "dbh")
all_lossvars = c(lossvars_stand, lossvars_rates)
# get all combinations of lossvars
lossvars_comb_rates = lapply(1:length(lossvars_rates), function(x) unlist(combn(lossvars_rates, x, simplify = T)))
lossvars_comb_rates = unlist(lapply(lossvars_comb_rates, function(x) apply(x,2, function(y) paste0(y, collapse="."))))

lossvars_comb_stand = lapply(1:length(lossvars_stand), function(x) unlist(combn(lossvars_stand, x, simplify = T)))
lossvars_comb_stand = unlist(lapply(lossvars_comb_stand, function(x) apply(x,2, function(y) paste0(y, collapse="."))))
lossvars_comb <- unlist(lapply(lossvars_comb_stand, function(x) unlist(lapply(lossvars_comb_rates, function(y) paste0(paste0(x,"."), y, collapse = ".")))))

lossvars_comb <- lossvars_comb[grepl("ba", lossvars_comb)]
# lossvars_comb[(grepl("growth.mort.reg", lossvars_comb))]
# lossvars_comb[(grepl("ba.trees.dbh.", lossvars_comb))]
