...
for(k in 0:5){
  ...
  m_list[[paste0("k",k)]][["train"]] <- train
  m_list[[paste0("k",k)]][["centers_test"]] <- centers_test
  m_list[[paste0("k",k)]][["scales_test"]] <- scales_test
  m_list[[paste0("k",k)]][["centers_train"]] <- centers_train
  m_list[[paste0("k",k)]][["scales_train"]] <- scales_train
  
  # tmp = test
  # tmp = train
  tmp_list = list(test = test, train = train)
  predictions = list("test" = NULL, "train" = NULL)
  for(i_tmp in c("test", "train")){
    for(i in 2:6) {
      tmp = tmp_list[[i_tmp]]
      centers = m_list[[paste0("k",k)]][[paste0("centers_",i_tmp)]]
      scales = m_list[[paste0("k",k)]][[paste0("scales_",i_tmp)]]
      pred = predict(m, newdata = tmp[tmp$year.x == i,])
      pred[,3] = exp(pred[,3])
      pred[,6] = exp(pred[,6])
      pred[pred<0] = 0
      colnames(pred) = c("dbh.x", "ba.x", "trees.x", "growth.x", "mort.x", "reg.x")
      if(i < 6) tmp[tmp$year.x == (i+1),10:15] = (pred - matrix(centers[1:6], nrow = nrow(pred), ncol = 6L, byrow = TRUE) )/ matrix(scales[1:6], nrow = nrow(pred), ncol = 6L, byrow = TRUE) 
      pred = data.frame(pred)
      pred$siteID = tmp[tmp$year.x == i,]$siteID
      pred$species = tmp[tmp$year.x == i,]$species
      pred$year = i
      predictions[[i_tmp]] = rbind(predictions[[i_tmp]], pred)
    }
  }
  # Add year 1 observed values (initial condition, no prediction possible)
  for(i_tmp in c("test", "train")){
    tmp = tmp_list[[i_tmp]]
    yr1 = tmp[tmp$year.x == 2, ]
    centers = m_list[[paste0("k",k)]][[paste0("centers_",i_tmp)]]
    scales = m_list[[paste0("k",k)]][[paste0("scales_",i_tmp)]]
    yr1_obs = data.frame(
      dbh.x    = yr1$dbh.y * scales[1] + centers[1],
      ba.x     = yr1$ba.y * scales[2] + centers[2],
      trees.x  = yr1$trees.y * scales[3] + centers[3],
      growth.x = yr1$growth.y * scales[4] + centers[4],
      mort.x   = yr1$mort.y * scales[5] + centers[5],
      reg.x    = yr1$reg.y * scales[6] + centers[6],
      siteID   = yr1$siteID,
      species  = yr1$species,
      year     = 1
    )
    predictions[[i_tmp]] = rbind(yr1_obs, predictions[[i_tmp]])
  }
  
  preds_test = as.data.table(predictions[["test"]])[,.(dbh.x = mean(dbh.x), ba.x = mean(ba.x), trees.x = mean(trees.x), growth.x = mean(growth.x), mort.x = mean(mort.x), reg.x = mean(reg.x)), by = .(species, year)]
  preds_train = as.data.table(predictions[["train"]])[,.(dbh.x = mean(dbh.x), ba.x = mean(ba.x), trees.x = mean(trees.x), growth.x = mean(growth.x), mort.x = mean(mort.x), reg.x = mean(reg.x)), by = .(species, year)]
  ...
  # Add year 1 true values from the .y columns (unscaled)
  yr1_true = as.data.table(test[test$year.x == 2, ])
  yr1_true = yr1_true[, .(
    dbh.x = mean(dbh.y) * scales_test[1] + centers_test[1],
    ba.x = mean(ba.y) * scales_test[2] + centers_test[2],
    trees.x = mean(trees.y) * scales_test[3] + centers_test[3],
    growth.x = mean(growth.y) * scales_test[4] + centers_test[4],
    mort.x = mean(mort.y) * scales_test[5] + centers_test[5],
    reg.x = mean(reg.y) * scales_test[6] + centers_test[6]
  ), by = .(species)]
  yr1_true[, year.x := 1]
  true = rbind(yr1_true, true)
  
  preds_all <- rbind(preds_all, data.table(preds_train, k = k, test_train = "train"))
  preds_all <- rbind(preds_all, data.table(preds_test, k = k, test_train = "test"))
...
for(i_tmp in c("test", "train")){
  for(k in 0:5){
    m = m_list[[paste0("k",k)]]$m
    test = m_list[[paste0("k",k)]]$test
    # centers_train = m_list[[paste0("k",k)]]$centers_train
    # scales_train = m_list[[paste0("k",k)]]$scales_train
    centers = m_list[[paste0("k",k)]][[paste0("centers_",i_tmp)]]
    scales = m_list[[paste0("k",k)]][[paste0("scales_",i_tmp)]]
    ...
    for(i in 1:max(env_dt_long$year.x)) {
      ...
      colnames(pred) = c("dbh.x", "ba.x", "trees.x", "growth.x", "mort.x", "reg.x")
      if(i < max(env_dt_long$year.x)) tmp[tmp$year.x == (i+1),10:15] = (pred - matrix(centers[1:6], nrow = nrow(pred), ncol = 6L, byrow = TRUE) )/ matrix(scales[1:6], nrow = nrow(pred), ncol = 6L, byrow = TRUE) 
      pred = data.frame(pred)
      pred$siteID = tmp[tmp$year.x == i,]$siteID
      pred$species = tmp[tmp$year.x == i,]$species
      pred$year = i
      
      predictions = rbind(predictions, data.table(pred, k = k, test_train = i_tmp))
      cat("\n", "k =",k, i_tmp)
    }
  }
}

preds_long_dt <- melt(data.table(predictions), id.vars = c("species", "year","siteID", "k", "test_train"))
preds_long_dt <- preds_long_dt[, .(value = mean(value, na.rm = T)), by = .(variable,species, year, k, test_train)]
...
library(ggplot2)
ggplot(preds_long_dt[test_train == "test"], aes(x = year, y = value, group = interaction(k,species2)))+
  geom_line(aes(color = factor(species2)))+
  facet_wrap(variable~k, scales = "free_y")+
  scale_color_manual(name = "PFT", values = pft_cols)
  # coord_cartesian(ylim = c(0,20))

preds_long_dt[, cv := paste0("S",k,"T0"), ]
...
comb_dt = melt(comb_dt, id.vars = c("species", "year", "k", "test_train"))
comb_dt[, var := tstrsplit(variable,"\\.")[[1]],]
comb_dt[, simpred := tstrsplit(variable,"\\.")[[2]],]

comb_dt_allspecies <- rbind(
  comb_dt[var %in% c("ba", "trees"), .(value = sum(value)/0.1), by = .(year,k,var,simpred,test_train)],
  comb_dt[var %in% c("dbh", "growth", "mort", "reg"), .(value = mean(value)), by = .(year,k,var,simpred,test_train)]
  )
comb_dt_allspecies$species = "all"

comb_dt <- rbind(comb_dt[,-"variable"], comb_dt_allspecies)

comb_dt <- dcast(comb_dt, species + year + k + var + test_train ~ simpred, value.var = "value")
cors_dt <- comb_dt[,.(
  r = cor(pred,true),
  rmse = sqrt(mean((pred-true)^2))
  ), by = .(var, species, k, test_train)]
...
table(comb_dt$year)
p_dt <- melt(comb_dt, id.vars = c("species", "year", "k", "var", "test_train"), variable.name = "pred_obs")
p_dt[, cv := paste0("S",k,"T0"), ]
fwrite(p_dt, "results/naive_simulated.csv")
ggplot(p_dt[test_train == "test"], aes(x=year,y=value,shape = pred_obs, color = factor(species)))+
  geom_point()+
  geom_line(mapping = aes(linetype = pred_obs))+
  facet_wrap(cv~var, scales = "free")
  
cors_dt[, cv := paste0("S",k,"T0"), ]
fwrite(cors_dt, "results/cors_naive.csv")
