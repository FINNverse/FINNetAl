...
for(i in 1:length(m_list)){
  ...
  if(grepl("1patch$", i_name)){
    Npatches = 1L
  } else if(grepl("9patches$", i_name)){
    Npatches = 9L
  }

  # read full raw data
  obs_dt = fread(paste0("data/Uholka/noSplits/", i_name,"/obs_dt.csv"))
  env_dt = fread(paste0("data/Uholka/noSplits/", i_name,"/env_dt.csv"))
  if(ncol(m$parameters_r$nn_regeneration.0.weight) == 4) env_dt = env_dt[,-c("Psd","Psum")]
  cohorts_dt = fread(paste0("data/Uholka/noSplits/", i_name,"/initial_cohorts2000.csv"))
  ...
  species_cors_dt <- 
    comp_allspecies_dt[,.(
      ...
      reg = cor(reg.obs, reg.pred, use = "complete.obs", method = "spearman"),
      mort = cor(mort.obs, mort.pred, use = "complete.obs", method = "spearman"),
      ba = cor(ba.obs, ba.pred, use = "complete.obs", method = "spearman"),
      trees = cor(trees.obs, trees.pred, use = "complete.obs", method = "spearman"),
      dbh = cor(dbh.obs, dbh.pred, use = "complete.obs", method = "spearman")
  ), by = species]
  # plot species_cors_dt as barplot with base r
  par(mfrow = c(2, 3))
  barplot(species_cors_dt$growth, names.arg = species_cors_dt$species, main = "growth", xlab = "species", ylab = "correlation", ylim = c(-1,1))
  barplot(species_cors_dt$reg, names.arg = species_cors_dt$species, main = "reg", xlab = "species", ylab = "correlation", ylim = c(-1,1))
  barplot(species_cors_dt$mort, names.arg = species_cors_dt$species, main = "mort", xlab = "species", ylab = "correlation", ylim = c(-1,1))
  barplot(species_cors_dt$ba, names.arg = species_cors_dt$species, main = "ba", xlab = "species", ylab = "correlation", ylim = c(-1,1))
  barplot(species_cors_dt$trees, names.arg = species_cors_dt$species, main = "trees", xlab = "species", ylab = "correlation", ylim = c(-1,1))
  barplot(species_cors_dt$dbh, names.arg = species_cors_dt$species, main = "dbh", xlab = "species", ylab = "correlation", ylim = c(-1,1))
